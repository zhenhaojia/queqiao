...
CREATE POLICY "Vendors can view inquiries about themselves" ON vendor_inquiries
    FOR SELECT USING (
      EXISTS (
        SELECT 1 FROM vendors v 
        WHERE v.id = vendor_inquiries.vendor_id 
...
CREATE POLICY "Vendors can view bookings about themselves" ON vendor_bookings
    FOR SELECT USING (
      EXISTS (
        SELECT 1 FROM vendors v 
        WHERE v.id = vendor_bookings.vendor_id 
        AND v.user_id = auth.uid()
      )
    );
